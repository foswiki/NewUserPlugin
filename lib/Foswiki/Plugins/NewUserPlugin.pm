...
sub beforeCommonTagsHandler {
    ...
    unless ( $wikiName =~ /^($wikiWordRegex)$/ ) {
        writeDebug(
"user's wikiname '$wikiName' is not a WikiWord ... not creating a homepage"
        );
        return;
    }
...
sub createUserTopic {
    ...
    my $registrationAgentCUID = Foswiki::Func::getCanonicalUserID(
...
# expanding VARs in a second phase, after the topic file was created (to get correct $meta objects)
    ...
    $found = 1
      if $text =~
          s/\%EXPAND\{(.*?)\}\%/&expandVariables($1, $wikiName, $usersWeb)/ge;
    $found = 1
      if $text =~
s/\%STARTEXPAND\%(.*?)\%STOPEXPAND\%/Foswiki::Func::expandCommonVariables($1, $wikiName, $usersWeb)/ges;
    ...
    if ($found) {
        ...
        if ($errorMsg) {
            writeWarning(
"error during save of var expanded version of $usersWeb.$wikiName: $errorMsg"
            );
        }
    }
    $session->{user} = $origCUID;
